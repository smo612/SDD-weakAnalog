...
OUT_ROOT = REPO / "results_probe_wsq_0506_cat7"
...
REMOTE_IMG = "data/cat/cat7_center_square.png"
REPAIR_TARGET = -35
...
def patch_config(asic_nsym: int, seed_value: int) -> str:
    ...
    if count_nsym != 1 or count_seed != 1:
        raise RuntimeError("Failed to patch config.py for wsq cat7 probe")
...
def run_and_record(*, out_dir: Path, rsi_scale: int, target_sinr_pre: int, run_tag: str, diffusion_t_start: int | None) -> dict:
    ...
    summary["protocol"] = "wsq_0506_cat7_resize128_square_remote"
