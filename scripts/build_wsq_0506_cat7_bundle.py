...
SOURCE = REPO / "results_probe_wsq_0506_cat7"
OUT_ROOT = REPO / "results_wsq_0506_cat7"
...
def write_notes(rows: list[dict]) -> None:
    ...
    meta = {
        ...
        "remote_presentation_fix": "cat7 was center-cropped offline to a square before the standard resize_128 protocol.",
        "repair_rule": "Use T_START=2 for weak1700 diffusion at -35 dB only; all other cat7 weak points remain baseline.",
    }
    (OUT_NOTES / "metadata.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")


def write_readme() -> None:
    text = """# WSQ 0506 Cat7 Bundle

Weak1700 integer-SINR cat7 bundle using a square center-cropped remote image.

- local: `kodim18`
- remote: `cat7_center_square.png`
...
def main() -> None:
    ...
    plot_metric(rows, "psnr", "wsq_0506_cat7_psnr.png", "PSNR Comparison")
    plot_metric(rows, "true_ms_ssim_db", "wsq_0506_cat7_msssim_db.png", "MS-SSIM Comparison")
    visual_grid(rows, "psnr", "PSNR", "wsq_0506_cat7_visual_grid_psnr.png")
    visual_grid(rows, "true_ms_ssim_db", "MS-SSIM", "wsq_0506_cat7_visual_grid_msssim.png")
    write_notes(rows)
    write_readme()
