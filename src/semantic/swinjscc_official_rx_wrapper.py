...
from src.semantic.swinjscc_official_wrapper import MODEL_SPECS


class SwinJSCCOfficialRXWrapper:
    """Decoder wrapper matching the official half-split SwinJSCC packing."""
    ...
    def decode(
        ...
        y_hat = self._symbols_to_latent_half_split(y_scaled, latent_shape)
        with torch.no_grad():
            y_tensor = torch.from_numpy(y_hat).to(self.device)
            x_hat = self.decoder(y_tensor, self.spec["given_snr"], self.spec["model_name"])
    ...
    def _symbols_to_latent_half_split(self, y_data: np.ndarray, latent_shape):
        ...
        flat = np.concatenate(
            [np.real(y).astype(np.float32), np.imag(y).astype(np.float32)]
        )
